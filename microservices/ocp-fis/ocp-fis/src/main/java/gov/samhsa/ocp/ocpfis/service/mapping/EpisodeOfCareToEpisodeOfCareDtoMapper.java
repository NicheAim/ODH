...
public class EpisodeOfCareToEpisodeOfCareDtoMapper {
    ...
    public static ReferenceDto mapToReferenceDto(Task task) {
        ...
        dto.setReference(task.getEncounter().getReference());

        if (task.hasEncounter()) {
            EpisodeOfCare episodeOfCare = (EpisodeOfCare) task.getEncounter().getResource();
    ...
    private static String createDisplayForEpisodeOfCare(Task task, EpisodeOfCare episodeOfCare) {
        ...
        try {
            activityDefinitionRefDto = convertReferenceToReferenceDto(task.getFocus());
