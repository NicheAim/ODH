...
public class ConsentPdfGeneratorWithHexPdfImpl implements ConsentPdfGenerator {
    ...
    private String getchars(String id, int width) {
        int size = 0;
        // 6.666666667 per width value
        if (width == 40) size = 6;
        if (width == 160) size = 24;
        String regex_split = "(?<=\\G.{"+size+"})";
        String[] chunked = id.split(regex_split);
        return String.join("\n", chunked);
    }
    ...
    private void drawActorsTable(HexPDF document, List<AbstractCareTeamDto> actors) {
        ...
        for (int i = 0; i < actors.size(); i++) {
            ...
            tableContentsForPractitioners[i + 1][1] = getchars(actors.get(i).getId(), 40);
    ...
    private void drawCareTeamTable(HexPDF document, List<ReferenceDto> actors) {
        ...
        for (int i = 0; i < actors.size(); i++) {
            ...
            tableContentsForCareTeams[i + 1][2] = getchars(actors.get(i).getReference().replace("CareTeam/", ""), 160);
        }
