...
public class AppointmentServiceImpl implements AppointmentService {
    ...
    public PageDto<AppointmentDto> getAppointments(Optional<List<String>> statusList,
        ...
        int numberOfAppointmentsPerPage = PaginationRepository.getValidPageSize(fisProperties, pageSize, ResourceType.Appointment.name());
        ...
        firstPageAppointmentBundle = PaginationRepository.getSearchBundleFirstPage(iQuery, numberOfAppointmentsPerPage, Optional.empty());
        ...
        if (pageNumber.isPresent() && pageNumber.get() > 1 && otherPageAppointmentBundle.getLink(Bundle.LINK_NEXT) != null) {
            ...
            if (fisProperties.getFhir().getData_store_tech().equals("gcp")) {
                otherPageAppointmentBundle = paginationGcp.getSearchBundleAfterFirstPage(fhirClient, fisProperties, firstPageAppointmentBundle, pageNumber.get(), numberOfAppointmentsPerPage);

            } else if (fisProperties.getFhir().getData_store_tech().equals("hapi")) {
                otherPageAppointmentBundle = paginationHapi.getSearchBundleAfterFirstPage(fhirClient, fisProperties, firstPageAppointmentBundle, pageNumber.get(), numberOfAppointmentsPerPage);

            } else {
                otherPageAppointmentBundle = paginationProxy.getSearchBundleAfterFirstPage(fhirClient, fisProperties, firstPageAppointmentBundle, pageNumber.get(), numberOfAppointmentsPerPage);
    ...
    public List<AppointmentParticipantReferenceDto> getAllHealthcareServicesReferences(String resourceType, String resourceValue) {
        ...
        return FhirOperationUtil.getAllBundleComponentsAsList(bundle, Optional.empty(), fhirClient, fisProperties).stream().map(entry -> {
            ...
            referenceDto.setReference(ResourceType.HealthcareService + "/" + hs.getIdElement().getIdPart());
    ...
    public List<AppointmentParticipantReferenceDto> getAllLocationReferences(String resourceType, String resourceValue) {
        ...
        if (SearchKeyEnum.LocationAppointmentParticipantSearchKey.HEALTHCARESERVICE.name().equalsIgnoreCase(resourceType)) {
            ...
            locationsRef = FhirOperationUtil.getAllBundleComponentsAsList(bundle, Optional.empty(), fhirClient, fisProperties).stream().map(entry -> {
                ...
                referenceDto.setReference(ResourceType.Location + "/" + location.getIdElement().getIdPart());
                setParticipantTypeAsAttender(referenceDto);
                setParticipantRequiredAsInformationOnly(referenceDto);
                setParticipantStatusAsAccepted(referenceDto);
                return referenceDto;
    ...
    private List<AppointmentParticipantReferenceDto> getAllPractitionersInOrganization(String organization) {
        int numberOfPractitionersPerPage = PaginationRepository.getValidPageSize(fisProperties, Optional.empty(), ResourceType.Practitioner.name());

        //Get the practitioners
        Bundle practitionerBundle = fhirClient.search().forResource(PractitionerRole.class)
    ...
    private Appointment setAppointmentStatusBasedOnParticipantActions(Appointment apt) {
        ...
        if (aptDto.getStatusCode().trim().equalsIgnoreCase(AppointmentConstants.PENDING_APPOINTMENT_STATUS)) {
            boolean allParticipantsHaveResponded = participantList.stream().noneMatch(temp -> temp.getParticipationStatusCode().trim().equalsIgnoreCase(AppointmentConstants.NEEDS_ACTION_PARTICIPATION_STATUS) &&
    ...
    private Appointment setParticipantAction(Appointment apt, String actorReference, String actionInUpperCase) {
        List<org.hl7.fhir.r4.model.Appointment.AppointmentParticipantComponent> participantList = apt.getParticipant();
        try {
            for (org.hl7.fhir.r4.model.Appointment.AppointmentParticipantComponent temp : participantList) {
